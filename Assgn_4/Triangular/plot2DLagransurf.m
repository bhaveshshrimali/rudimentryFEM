function plot2DLagransurf(P, nel, edge, knots, numu, gridlin, ID)
...
%
% Copyright (C) Arif Masud and Tim Truster% Tim Truster
% UIUC
% 06/22/2009
%
%
...
for j = 1:numu
    for i = 1:numu
        ...
        r = knots(ind,1);
        s = knots(ind,2);
        SurPoint = LagrSurfacePoint(P,nel,edge,r,s,2);
        for dir = 1:2
...
%Plot the list of curve points, control points, and knots
hold on
surf(SPL2D(:,:,1),SPL2D(:,:,2),zeros(numu,numu),'EdgeColor',gridlin,'FaceColor',[0 0 1])
uinc = round((numu-1)/2)+1;
